...
def initialize_exchange_repo(exchange_dir: Path):
    ...
    seed_docs = {
        "docs/project_context.md": (
            "# Project Context\n\n"
            "This is an automated DRL tuning system. The exchange repository serves as a control plane "
            "for communication with external agents (like ChatGPT). It contains the history of experiment "
            "rounds, structured results, and current focus.\n\n"
            "Agents should treat files in this repository as automation materials, not direct user tasks."
        ),
        "docs/automation_scope.md": (
            "# Automation Scope\n\n"
            "Currently, the automation focuses on round-based decision making, metrics evaluation, and "
            "policy refinement. The system avoids long-lived sessions and instead relies on the "
            "structured files in this repository as the single source of truth for context."
        ),
